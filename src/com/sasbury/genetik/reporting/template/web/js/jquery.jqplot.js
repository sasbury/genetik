...
(function($) {
    ...
    function Axis(name) {
        ...
        this.labelRenderer = $.jqplot.CanvasAxisLabelRenderer;
        // prop: labelOptions
    ...
    function Title(text) {
        ...
        // see <$.jqplot.CanvasTitleRenderer>.
        this.renderer = $.jqplot.CanvasTitleRenderer;
        // prop: rendererOptions
        // renderer specific options passed to the renderer.
        this.rendererOptions = {};  
        
        this._textRenderer = new $.jqplot.CanvasTextRenderer({fontSize: '1.1em'}); 
    }
    ...
    Title.prototype.getHeight = function(ctxt) {
           return this.renderer.getHeight.call(this,ctxt);
       };
    ...
    Series.prototype.init = function(index, gridbw, plot) {
        ...
        if(!this.markerOptions.style)
        {
            this.markerOptions.style = $.jqplot.nextMarker();
        }
    ...
    function Grid() {
        ...
        this.gridLineColor = '#AAA';
        // prop: gridLineWidth
        // width of the grid lines.
        this.gridLineWidth = 1.0;
        // prop: background
        // css spec for the background color.
        this.background = '#FFFFFF';//'#f3f3f3';
        // prop: borderColor
    ...
    function jqPlot() {
        ...
        this.seriesColors = [ "#1240AB", "#FFD300", "#FF7400", "#2A4480", "#BFA730", "#BF7130", "#06266F", "#A68900", "#A64B00", "#4671D5", "#FFDE40", "#FF9640"];
        this.negativeSeriesColors = [ "#1240AB", "#FFD300", "#FF7400", "#2A4480", "#BFA730", "#BF7130", "#06266F", "#A68900", "#A64B00", "#4671D5", "#FFDE40", "#FF9640"];
        // prop: sortData
        ...
        this.draw = function(){
            if (this.drawIfHidden || this.target.is(':visible')) {
                ...
                if (this.title.show && this.axes.x2axis.show) {
                    this._gridPadding.top = this.title.getHeight(bctx) + this.axes.x2axis.getHeight()+8;
                }
                else if (this.title.show) {
                    this._gridPadding.top = this.title.getHeight(bctx) + 8;
                }
                else if (this.axes.x2axis.show) {
                    this._gridPadding.top = this.axes.x2axis.getHeight();
                }
                if (this.axes.xaxis.show) {
                    this._gridPadding.bottom = this.axes.xaxis.getHeight() + 8;
                }
    ...
    */
    ...
    // Class: $.jqplot.CanvasTitleRenderer
    // The default title renderer for jqPlot.  This class has no options beyond the <Title> class. 
    $.jqplot.CanvasTitleRenderer = function() {
    };
    
    $.jqplot.CanvasTitleRenderer.prototype.init = function(options) {
        $.extend(true, this, options);
    };
    
    $.jqplot.CanvasTitleRenderer.prototype.draw = function(bctx) {
        var r = this.renderer;
        if (this.text) {
        	this._textRenderer.setText(this.text,bctx);
        	this._textRenderer.draw(bctx,this.text);
        }
        
        return this._elem;
    };
    
    $.jqplot.CanvasTitleRenderer.prototype.getHeight = function(ctx) {
        
        var tr = this._textRenderer;
        var l = tr.getWidth(ctx);
        var h = tr.getHeight(ctx);
        var h = Math.abs(Math.cos(tr.angle)*h) + Math.abs(Math.sin(tr.angle)*l);
        return h;
    };
    
    $.jqplot.CanvasTitleRenderer.prototype.pack = function() {
        // nothing to do here
    };
    ...
    $.jqplot.LineRenderer.prototype.draw = function(ctx, gd, options) {
        ...
        if (gd.length) {
            ...
            {
                var pxzero = this._xaxis.series_u2p(-100000);
                var pyzero = this._yaxis.series_u2p(-100000);
                ...
                for (i=0; i<gd.length; i++)
                {
                    ...
                    if(this.extendToY)
                    {
                        var points = [[gd[i][0],pyzero], [gd[i][0], gd[i][1]]];
                        this.renderer.shapeRenderer.draw(ctx, points, opts.markerOptions);
                    }
                    
					if(this.extendToX)
                    {
                        var points = [[pxzero,gd[i][1]], [gd[i][0], gd[i][1]]];
                        this.renderer.shapeRenderer.draw(ctx, points, opts.markerOptions);
                    }
                }
            }
    ...
    $.jqplot.LinearAxisRenderer.prototype.set = function() { 
        ...
        if (this.show && this.showTicks) {
            ...
            if (this.name == 'xaxis') {
                ...
                if (lshow && this._label.constructor == $.jqplot.CanvasAxisLabelRenderer) {
    ...
    $.jqplot.LinearAxisRenderer.prototype.createTicks = function() {
        ...
        // we don't have any ticks yet, let's make some!
            ...
            for (var i=0; i<this.numberTicks; i++){
                ...
                var t = new this.tickRenderer(this.tickOptions);
                // var t = new $.jqplot.CanvasAxisTickRenderer(this.tickOptions);
                if (!this.showTicks) {
                    t.showLabel = false;
                    t.showMark = false;
                }
    ...
    $.jqplot.curMarkerindex = 0;
    $.jqplot.markerChoices = ['filledDiamond','filledTriangle','filledSquare','filledCircle','filledInvertedTriangle','square','triangle','diamond','circle','invertedTriangle','plus','dash']
    $.jqplot.nextMarker = function()
    {
        var retVal = $.jqplot.markerChoices[$.jqplot.curMarkerindex];
        $.jqplot.curMarkerindex++;
        $.jqplot.curMarkerindex = $.jqplot.curMarkerindex % $.jqplot.markerChoices.length;
        return retVal;
    }
   ...
   $.jqplot.MarkerRenderer.prototype.drawTriangle = function(x, y, ctx, fill, options) {
        var stretch = 1.0;
        var dx = this.size/2/stretch;
        var dy = this.size/2*stretch;
        var points = [[x-dx, y+dy], [x, y-dy], [x+dx, y+dy]];
        if (this.shadow)
		{
            this.shadowRenderer.draw(ctx, points);
        }
   ...
   $.jqplot.MarkerRenderer.prototype.drawInvertedTriangle = function(x, y, ctx, fill, options) {
        var stretch = 1.0;
        var dx = this.size/2/stretch;
        var dy = this.size/2*stretch;
        var points = [[x-dx, y-dy], [x, y+dy], [x+dx, y-dy]];
    ...
    $.jqplot.MarkerRenderer.prototype.draw = function(x, y, ctx, options) {
        ...
        switch (this.style) {
            ...
            case 'filledSquare':
                ...
                break;
            case 'triangle':
                this.drawTriangle(x,y,ctx, false, options);
                break;
            case 'filledTriangle':
                this.drawTriangle(x,y,ctx, true, options);
                break;
            case 'invertedTriangle':
                this.drawInvertedTriangle(x,y,ctx, false, options);
                break;
            case 'filledInvertedTriangle':
                this.drawInvertedTriangle(x,y,ctx, true, options);
                break;
            case 'x':
